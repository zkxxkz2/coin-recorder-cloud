...
### v6.4.2 (最新版本)
- **添加应用入口优化**：右上角“添加到桌面”按钮增加文字提示“添加应用”，提升可发现性。
...
### v6.3.2
- **周/月统计口径调整**：周、月图表改为显示“当周/当月积攒的金币（净增量）”，按期间内 difference 累加
- **图表文案更新**：将周/月图表标签与标题分别更新为“周积攒金币/周统计图表（积攒）”与“月积攒金币/月统计图表（积攒）”

### v6.3.1
- **修复排行榜自动刷新问题**：登录账号后不再自动获取排行榜数据
- **手动刷新控制**：排行榜数据需要用户手动点击刷新按钮才会获取
- **节省流量**：避免登录时自动请求排行榜数据，减少不必要的网络请求

### v6.3.0
- **首页布局优化**：重新排列卡片顺序，提升用户体验
- **挑战进度置顶**：将挑战进度卡片放在最顶端，突出目标导向
- **快速记录第二**：快速记录卡片移至第二位，便于快速操作
- **今日统计第三**：今日统计数据紧跟记录功能
- **连击统计第四**：连击统计放在最后，作为激励展示
